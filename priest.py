...
class Mag(person.Person):
    ...
    def __init__(self, name: str) -> None:
        ...
        #Я ещё не сделал чистку от дибаффов в движке (а может и сделал, я уже не помню)
        cast_bonus = {'evasion': 100}  
        def finish(name: str, damage: float) -> dict:
            result = {'target': {}, 'self': []}
            result['self'].append({
        ...
        Purification = person.classSkill.Skill(name='Purification', cast=1, coldown=5, lvl=4, finish=finish, mult=1)
        ...
        def finish(name: str, damage: float) -> dict:
            ...
            if random.randint(0, 20) < 5: result['target'][-1]['defense'] = 100 * self.lvl
            if random.randint(0, 20) < 5: result['target'][-1]['resistance'] = 100 * self.lvl
            if random.randint(0, 20) < 5: result['target'][-1]['damage'] = 100 * self.lvl
            if random.randint(0, 20) < 5: result['target'][-1]['precision'] = 100 * self.lvl
            return result
